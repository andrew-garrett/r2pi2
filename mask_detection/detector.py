...
def proj2face_space(target_vector, eigenfaces):
    ...
    weight = np.matmul(eigenfaces, target_vector.T)
    weight = weight / np.sum(weight)
    proj = np.matmul(weight.T, eigenfaces)
    return proj;
#pass

def dist2face_space(target_vector, face_space_vector):
    # Copy and paste your code from notebook
    face_dist = np.linalg.norm(face_space_vector - target_vector, 2)
    return face_dist;
#pass
...
#    with RPiCamera('tcp://192.168.1.84:65433') as camera:
#        #mask_detection(camera)
#        k = 20
#        thresh = 47000
#        pca_face_detection(camera, k, thresh)