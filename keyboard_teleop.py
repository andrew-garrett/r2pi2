...
hex_to_rgb = {
    ...
    '#FFA500': Color(255, 165, 0),
    '#FFFF00': Color(255, 255, 0),
    '#008000': Color(0, 128, 0),
    '#0000FF': Color(0, 0, 255),
    '#4B0082': Color(75, 0, 130),
    '#8F00FF': Color(238, 130, 238),
    '#800080': Color(128, 0, 128)
}
...
def sort_lambda(roll_commands: List[Tuple[int, int, float]]):
    roll_commands.sort(lambda x : (x[2], x[1]))
...
def drive_polygon(droid: SpheroEduAPI, n: int, speed: int = 100, duration: float = 1.5):
    ...
    for i in range(n):
        droid.roll(i * internal_angle, speed, duration)
...
def set_lights(droid: SpheroEduAPI, color: str, which_light='both'):
    ...
    if color[0] == "#":
        ...
        col = color_names_to_rgb.get(color)
    if which_light in ['front', 'both']:
        droid.set_front_led(col)
    if which_light in ['back', 'both']:
        droid.set_back_led(col)


def hex2rgb(hex_code: str) -> Color:
    """convert HEX to RGB, one line"""
    return Color(int(hex_code[1:3], 16), int(hex_code[3:5], 16), int(hex_code[5:], 16))
...
def drive_with_keyboard(droid: SpheroEduAPI, speed_delta: int = 10, heading_delta: int = 30, dome_delta: int = 20):
    ...
    while True:
        ...
        if key == 'esc':
            ...
            droid.set_heading((droid.get_heading() - heading_delta) % 360)
        elif key == 'right':
            droid.set_heading((droid.get_heading() + heading_delta) % 360)
        elif key == 'tab':
            curr_dome = (curr_dome + dome_delta) % 360
            droid.set_dome_position(curr_dome)
        elif key == 'return':
            curr_dome = (curr_dome - dome_delta) % 360
            droid.set_dome_position(curr_dome)
        elif key == 'space':
            bipod_val = not bipod_val
            droid.set_stance(Stance.Bipod if bipod_val else Stance.Tripod)
...
def encode_in_morse_code(message: str) -> Iterator[str]:
    for chr in message:
        if chr.upper() in morse_code_dict:
            yield morse_code_dict[chr.upper()]
...
def play_message(droid: SpheroEduAPI, message: str, dot_duration: float = 0.1, dash_duration: float = 0.3,
    ...
    msg = encode_in_morse_code(message)

    for m in msg:
        for chr in m:
            blink(droid, dot_duration if chr == "." else dash_duration)
...
def start_surveillance(droid: SpheroEduAPI):
    ...
    with RPiCamera('tcp://172.20.10.10:65433') as camera:  # Replace IP_ADDRESS with the IP address of your Raspberry Pi
